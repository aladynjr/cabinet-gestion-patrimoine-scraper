...
const resultFolder = 'result_familyoffice';
...
const scrapeWebsite = async () => {
  const baseUrl = 'https://www.familyoffice-france.fr/?35cdbc78_page=';
  let totalListings = 0;
  const allListings = [];
  ...
  for (let page = 1; page <= 7; page++) {
    let config = {
      ...
      headers: { 
        ...
        'pragma': 'no-cache', 
        'priority': 'u=1, i', 
        'referer': 'https://www.familyoffice-france.fr/?35cdbc78_page=5', 
...
const scrapeListings = async (listings) => {
  const baseUrl = 'https://www.familyoffice-france.fr';
  const scrapedData = [];
  ...
  for (let i = 0; i < listings.length; i++) {
    ...
    try {
      ...
      const jsonLdScript = $('script[type="application/ld+json"]').filter((_, el) => {
        ...
        return content && content.includes('"@type": "LocalBusiness"') && content.includes('"telephone":');
      }).html();
...
const main = async () => {
  try {
    ...
    const scrapedListingsData = await fs.readFile(path.join(resultFolder, 'scraped_listings.json'), 'utf8');
    const scrapedListings = JSON.parse(scrapedListingsData);
